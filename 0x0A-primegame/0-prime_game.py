...
def isWinner(x, nums):
    ...
    if x != len(nums):
        return None
    # Initialize scores and possible prime numbers array
    ben = 0
    maria = 0
    # Create 'a' of length sorted(nums)[-1] + 1 list with all elements
    # initialized to 1
    a = [1 for x in range(sorted(nums)[-1] + 1)]
    # The first two elements of the list, a[0] and a[1], are set to 0
    # because 0 and 1 are not prime numbers
    a[0], a[1] = 0, 0
    # Use Sieve of Eratosthenes algorithm to generate prime numbers array
...
def rm_multiples(ls, x):
    ...
    Raises:
        None.
    """
    # This loop iterates over multiples prime numbers and marks them as
